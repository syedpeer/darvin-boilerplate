...
// Set prompt as green
prompt.message = colors.green("Edit");



const spinner = ora('Prepare Scaffolding');
spinner.color = 'red';
spinner.start();
...
module.exports = (args, options, logger) => {
  ...
  if (fs.existsSync(templatePath)) {
    ...
    spinner.text = "Please add following informations:"
    spinner.info();
  } else {
    spinner.text = `The requested template for ${args.template} wasn’t found.`;
    spinner.fail();
    process.exit(1);
  }
  ...
  prompt.start().get(variables, (err, result) => {
    ...
    shell.ls('-Rl', '.').forEach(entry => {
      ...
      if (!spinnerPrompt) {
        spinner.stop();
        spinner.start('write project metaset');
        spinnerPrompt = true;
      }
